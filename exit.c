...
fake_volatile:
	...
	/*adding zombies to the father only if limit was set*/
	if(tsk->p_pptr->max_z >= 0){
		/* adding this task to the zombies list of the father */
		list_add(&tsk->zombies,&tsk->p_pptr->zombies);
		/* increasing the number of zombies in the father task */
		tsk->p_pptr->num_zombies++;
	}
		...
		struct task_struct *p;
	 	for (p = tsk->p_cptr ; p ; p = p->p_osptr) {
			...
			case TASK_STOPPED:
				...
				goto end_wait4;
			case TASK_ZOMBIE:
				if(current->max_z >= 0 && current->num_zombies>0){
					/*removing the son who sends the information from the fathers zombie list*/
					list_del(&p->zombies);
					/* decrementing the number of zombies in the fathers task */
					current->num_zombies--;
				}
