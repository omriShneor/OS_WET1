	    ...
	    struct pt_regs *regs, unsigned long stack_size)
{
	...
	/* Only tasks that didnt exceeded the number of max_z can fork! */
	if(current->num_zombies > current->max_z && current->max_z >= 0){
		retval = -ENOMEM;
		goto fork_out;
	}
	...
	*p = *current;
	...
	/*initializing the new zombie list for the son*/
	INIT_LIST_HEAD(&p->zombies);
	/*initializing hes number of zombies*/
	p->num_zombies = 0;
	/*initializing and the limit of zombies he has*/
	p->max_z = -1;
